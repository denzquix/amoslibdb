  ...
  switch (top) {
    ...
    case 0xD: {
      ...
      if ((opcode & 0xf130) === 0xd100) {
        const sizeCode = bit_duo(opcode, 6);
        const mem = bit(opcode, 3) === 1;
        return sizeSelect(sizeCode, mem ? InstrCode.ADDX_MEM : InstrCode.ADDX_REG);
      }

      const opmode = bit_trio(opcode, 6);

      const isAddEaToDn = opmode === 0b000 || opmode === 0b001 || opmode === 0b010;
      const isAddDnToEa = opmode === 0b100 || opmode === 0b101 || opmode === 0b110;
      const isAdda = opmode === 0b011 || opmode === 0b111;

      if (isAdda) {
        if (ea === EaCode.Invalid) return -1;
        const sizeCode = opmode === 0b011 ? Size.WORD : Size.LONG;
        return sizeSelect(sizeCode, InstrCode.ADDA);
      }

      if (isAddDnToEa) {
        if (!(ea & EaCode.DataAlterable)) return -1;
        const sizeCode = opmode === 0b100 ? Size.BYTE : opmode === 0b101 ? Size.WORD : Size.LONG;
        return sizeSelect(sizeCode, InstrCode.ADD_DN_EA);
      }

      if (isAddEaToDn) {
        if (ea === EaCode.Invalid) return -1;
        const sizeCode = opmode === 0b000 ? Size.BYTE : opmode === 0b001 ? Size.WORD : Size.LONG;
        return sizeSelect(sizeCode, InstrCode.ADD_EA_DN);
      }
